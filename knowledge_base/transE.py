...
class TransEModel:
    ...
    def _add_embeddings(self):
        ...
        self.relation_embeddings = tf.Variable(self.initializer(shape=[self.rel_size + 1, constants.INPUT_W2V_DIM]),
                                               dtype=tf.float32, name='relation', trainable=True)
        ...
        for e in range(self.epochs):
            ...
            if early_stopping:
                ...
                for idx, batch in enumerate(self.next_batch(data['head'], data['tail'], data['rel'], data['head_neg'],
                    ...
                    head, tail, rel, head_neg, tail_neg = batch
                    val_logits = self.model([head, tail, rel, head_neg, tail_neg], training=False)
                ...
                Log.log("Loss: {}".format(val_loss))
                print("Best Loss: ", best_loss)
                print("Loss for epoch number {}: {}".format(e + 1, val_loss))
